...
configfilename = '/home/nana/Desktop/workspace/OverlapNet/config/network.yml'
if len(sys.argv) > 1:
...
if 'training_seqs' in config:
    ...
    validationdata_npzfiles = [os.path.join(data_root_folder, seq, 'ground_truth/validation_set.npz')
...
fileHandler = logging.FileHandler("{0}/training.log".format(os.path.join(experiments_path, testname)),  mode='w')
fileHandler.setFormatter(logging.Formatter(fmt="%(asctime)s %(message)s", datefmt='%H:%M:%S'))
logger.addHandler(fileHandler)

# weights filename: if empty, not saved
weights_filename = os.path.join(os.path.join(experiments_path, testname), modelType + '_' + testname + '.weight')
...
network_output_size = model.get_layer('orientation_output_yaw').output_shape[1]
...
losses = {
    "overlap_output": my_sigmoid_loss,
    "orientation_output_yaw": my_entropy,
    "orientation_output_pitch": my_entropy,
    "orientation_output_roll": my_entropy,
    "translation_output_x": my_entropy,
    "translation_output_y": my_entropy,
    "translation_output_z": my_entropy
}

lossWeights = {
    "overlap_output": 1.0,
    "orientation_output_yaw": 1.0,
    "orientation_output_pitch": 1.0,
    "orientation_output_roll": 1.0,
    "translation_output_x": 1.0,
    "translation_output_y": 1.0,
    "translation_output_z": 1.0
}

model.compile(loss=losses, loss_weights=lossWeights, optimizer=optimizer)

logger.info("compiled model with learning_rate=%f, lr_alpha=%f, momentum=%f" % (initial_lr, lr_alpha, momentum))
...
(train_imgf1, train_imgf2, train_dir1, train_dir2, train_overlap, train_orientation_yaw, train_orientation_pitch,
 train_orientation_roll, train_translation_x, train_translation_y, train_translation_z) = \
...
if batch_size * no_batches_in_epoch < n:
    ...
    train_orientation_yaw = train_orientation_yaw[0:n]
    train_orientation_pitch = train_orientation_pitch[0:n]
    train_orientation_roll = train_orientation_roll[0:n]
    train_translation_x = train_translation_x[0:n]
    train_translation_y = train_translation_y[0:n]
    train_translation_z = train_translation_z[0:n]
else:
    no_batches_in_epoch = int(np.floor(n / float(batch_size)))

logger.info("load validation data ...")
(validation_imgf1, validation_imgf2, validation_dir1, validation_dir2, validation_overlap, validation_orientation_yaw,
 validation_orientation_pitch, validation_orientation_roll, validation_translation_x, validation_translation_y,
 validation_translation_z) = \
...
if no_test_pairs < validation_overlap.size:
    ...
    validation_orientation_yaw = validation_orientation_yaw[0:no_test_pairs]
    validation_orientation_pitch = validation_orientation_pitch[0:no_test_pairs]
    validation_orientation_roll = validation_orientation_roll[0:no_test_pairs]
    validation_translation_x = validation_translation_x[0:no_test_pairs]
    validation_translation_y = validation_translation_y[0:no_test_pairs]
    validation_translation_z = validation_translation_z[0:no_test_pairs]
else:
...
train_generator = ImagePairOverlapOrientationSequence(imgpath,
                                                      ...
                                                      train_dir2,
                                                      train_overlap,
                                                      train_orientation_yaw,
                                                      train_orientation_pitch,
                                                      train_orientation_roll,
                                                      train_translation_x,
                                                      train_translation_y,
                                                      train_translation_z,
                                                      network_output_size,
...
validation_generator = ImagePairOverlapOrientationSequence(imgpath,
                                                           ...
                                                           validation_dir2,
                                                           validation_overlap,
                                                           validation_orientation_yaw,
                                                           validation_orientation_pitch,
                                                           validation_orientation_roll,
                                                           validation_translation_x,
                                                           validation_translation_y,
                                                           validation_translation_z,
                                                           network_output_size,
                                                           batch_size,
                                                           inputShape[0],
...
for epoch in range(0, no_epochs):
    ...
    summary = tf.Summary(value=[tf.Summary.Value(tag=losstag14, simple_value=max_error)])
    writer.add_summary(summary, epoch)

    summary = tf.Summary(value=[tf.Summary.Value(tag=losstag15, simple_value=rms_error)])
    writer.add_summary(summary, epoch)

    # orientation RMS for different overlap thresholds
    summary = tf.Summary(value=[tf.Summary.Value(tag=losstag0, simple_value=epoch_loss)])
    writer.add_summary(summary, epoch)

    for j in range(0, len(batchLossHistory.losses)):
        summary = tf.Summary(value=[tf.Summary.Value(tag=losstag1, simple_value=batchLossHistory.losses[j])])
        writer.add_summary(summary, j + epoch * no_batches_in_epoch)

    summary = tf.Summary(value=[tf.Summary.Value(tag=losstag2, simple_value=learning_rate_hist)])
    writer.add_summary(summary, epoch)
    ...
    for overlap_thre in overlap_thres:
        ...
        for idx in range(len(model_outputs[1])):
            ...
            angle_yaw = np.argmax(model_outputs[1][idx])
            angle_pitch = np.argmax(model_outputs[2][idx])
            angle_roll = np.argmax(model_outputs[3][idx])

            translate_x = np.argmax(model_outputs[4][idx])
            translate_y = np.argmax(model_outputs[5][idx])
            translate_z = np.argmax(model_outputs[6][idx])

            overlap = np.max(model_outputs[0][idx])
            if overlap > overlap_thre:
                diffs_yaw.append(min(abs(angle_yaw - validation_orientation_yaw[idx]),
                                     network_output_size - abs(angle_yaw - validation_orientation_yaw[idx])))
                diffs_pitch.append(min(abs(angle_pitch - validation_orientation_pitch[idx]),
                                       network_output_size - abs(angle_pitch - validation_orientation_pitch[idx])))
                diffs_roll.append(min(abs(angle_roll - validation_orientation_roll[idx]),
                                      network_output_size - abs(angle_roll - validation_orientation_roll[idx])))

                diffs_x.append(min(abs(translate_x - validation_translation_x[idx]),
                                   network_output_size - abs(translate_x - validation_translation_x[idx])))
                diffs_y.append(min(abs(translate_y - validation_translation_y[idx]),
                                   network_output_size - abs(translate_y - validation_translation_y[idx])))
                diffs_z.append(min(abs(translate_z - validation_translation_z[idx]),
                                   network_output_size - abs(translate_z - validation_translation_z[idx])))

        diffs_yaw = np.array(diffs_yaw)
        diffs_pitch = np.array(diffs_pitch)
        diffs_roll = np.array(diffs_roll)

        diffs_x = np.array(diffs_x)
        diffs_y = np.array(diffs_y)
        diffs_z = np.array(diffs_z)

        if diffs_yaw.any():
            max_error_yaw = np.max(diffs_yaw)
            mean_square_error_yaw = np.mean(diffs_yaw * diffs_yaw)
            rms_error_yaw = np.sqrt(mean_square_error_yaw)

            logger.info("  Evaluation on test data results: ")
            logger.info("  Evaluation: mean Yaw difference:   %f" % mean_square_error_yaw)
            logger.info("  Evaluation: max  Yaw difference:   %f" % max_error_yaw)
            logger.info("  Evaluation: RMS  Yaw error     :   %f" % rms_error_yaw)

            max_error_pitch = np.max(diffs_pitch)
            mean_square_error_pitch = np.mean(diffs_pitch * diffs_pitch)
            rms_error_pitch = np.sqrt(mean_square_error_pitch)

            logger.info("  Evaluation on test data results: ")
            logger.info("  Evaluation: mean Pitch difference:   %f" % mean_square_error_pitch)
            logger.info("  Evaluation: max  Pitch difference:   %f" % max_error_pitch)
            logger.info("  Evaluation: RMS  Pitch error     :   %f" % rms_error_pitch)

            max_error_roll = np.max(diffs_yaw)
            mean_square_error_roll = np.mean(diffs_roll * diffs_roll)
            rms_error_roll = np.sqrt(mean_square_error_roll)

            logger.info("  Evaluation on test data results: ")
            logger.info("  Evaluation: mean Roll difference:   %f" % mean_square_error_roll)
            logger.info("  Evaluation: max  Roll difference:   %f" % max_error_roll)
            logger.info("  Evaluation: RMS  Roll error     :   %f" % rms_error_roll)

            max_error_x = np.max(diffs_x)
            mean_square_error_x = np.mean(diffs_x * diffs_x)
            rms_error_x = np.sqrt(mean_square_error_x)

            logger.info("  Evaluation on test data results: ")
            logger.info("  Evaluation: mean X difference:   %f" % mean_square_error_x)
            logger.info("  Evaluation: max  X difference:   %f" % max_error_x)
            logger.info("  Evaluation: RMS  X error     :   %f" % rms_error_x)

            max_error_y = np.max(diffs_y)
            mean_square_error_y = np.mean(diffs_y * diffs_y)
            rms_error_y = np.sqrt(mean_square_error_y)

            logger.info("  Evaluation on test data results: ")
            logger.info("  Evaluation: mean Y difference:   %f" % mean_square_error_y)
            logger.info("  Evaluation: max  Y difference:   %f" % max_error_y)
            logger.info("  Evaluation: RMS  Y error     :   %f" % rms_error_y)

            max_error_z = np.max(diffs_z)
            mean_square_error_z = np.mean(diffs_z * diffs_z)
            rms_error_z = np.sqrt(mean_square_error_z)

            logger.info("  Evaluation on test data results: ")
            logger.info("  Evaluation: mean Z difference:   %f" % mean_square_error_z)
            logger.info("  Evaluation: max  Z difference:   %f" % max_error_z)
            logger.info("  Evaluation: RMS  Z error     :   %f" % rms_error_z)

            summary = tf.Summary(
                value=[tf.Summary.Value(tag='orientation "Yaw" RMS different overlap thresholds/' + str(overlap_thre),
                                        simple_value=rms_error_yaw),
                       tf.Summary.Value(tag='orientation "Pitch" RMS different overlap thresholds/' + str(overlap_thre),
                                        simple_value=rms_error_pitch),
                       tf.Summary.Value(tag='orientation "Roll" RMS different overlap thresholds/' + str(overlap_thre),
                                        simple_value=rms_error_roll),
                       tf.Summary.Value(tag='translation "X" RMS different overlap thresholds/' + str(overlap_thre),
                                        simple_value=rms_error_x),
                       tf.Summary.Value(tag='translation "Y" RMS different overlap thresholds/' + str(overlap_thre),
                                        simple_value=rms_error_y),
                       tf.Summary.Value(tag='translation "Z" RMS different overlap thresholds/' + str(overlap_thre),
                                        simple_value=rms_error_z),
                       ])
            writer.add_summary(summary, epoch)
