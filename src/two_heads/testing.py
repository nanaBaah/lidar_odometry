...
configfilename = '/home/nana/Desktop/workspace/OverlapNet/config/network.yml'  # 'network.yml'
if len(sys.argv) > 1:
...
if 'testing_seqs' in config:
    ...
    testdata_npzfiles = [os.path.join(data_root_folder, seq, 'ground_truth/validation_set.npz') for seq in
                         training_seqs]
...
orientation_predict_yaw = network_generate_method_orientation_head(inputHead_l, inputHead_r,
                                                                   'orientation_output_yaw', config['model'])
orientation_predict_pitch = network_generate_method_orientation_head(inputHead_l, inputHead_r,
                                                                     'orientation_output_pitch', config['model'])
orientation_predict_roll = network_generate_method_orientation_head(inputHead_l, inputHead_r,
                                                                    'orientation_output_roll', config['model'])
orientation_predict_x = network_generate_method_orientation_head(inputHead_l, inputHead_r,
                                                                 'translation_output_x', config['model'])
orientation_predict_y = network_generate_method_orientation_head(inputHead_l, inputHead_r,
                                                                 'translation_output_y', config['model'])
orientation_predict_z = network_generate_method_orientation_head(inputHead_l, inputHead_r, 'translation_output_z',
                                                                 config['model'])

head = keras.Model(inputs=(inputHead_l, inputHead_r),
                   outputs=[overlap_predict,
                            orientation_predict_yaw,
                            orientation_predict_pitch,
                            orientation_predict_roll,
                            orientation_predict_x,
                            orientation_predict_y,
                            orientation_predict_z])

logger.info("Created neural net %s for leg   with %d parameters." % (modelType, leg.count_params()))
logger.info("Created neural net %s for heads with %d parameters." % (modelType, head.count_params()))
...
(test_imgf1, test_imgf2, test_dir1, test_dir2, test_overlap, test_orientation_yaw, test_orientation_pitch,
 test_orientation_roll, test_orientation_x, test_orientation_y, test_orientation_z) = \
...
test_orientation_yaw = test_orientation_yaw[0:no_test_pairs]
test_orientation_pitch = test_orientation_pitch[0:no_test_pairs]
test_orientation_roll = test_orientation_roll[0:no_test_pairs]

test_orientation_x = test_orientation_x[0:no_test_pairs]
test_orientation_y = test_orientation_y[0:no_test_pairs]
test_orientation_z = test_orientation_z[0:no_test_pairs]
...
test_generator_leg = ImagePairOverlapOrientationSequence(image_path=imgpath,
                                                         imgfilenames1=test_allimgs,
                                                         imgfilenames2=[],
                                                         dir1=[test_dir for _ in range(len(test_allimgs))],
                                                         dir2=[],
                                                         overlap=np.zeros((len(test_allimgs))),
                                                         orientation_yaw=np.zeros((len(test_allimgs))),
                                                         orientation_pitch=np.zeros((len(test_allimgs))),
                                                         orientation_roll=np.zeros((len(test_allimgs))),
                                                         translation_x=np.zeros((len(test_allimgs))),
                                                         translation_y=np.zeros((len(test_allimgs))),
                                                         translation_z=np.zeros((len(test_allimgs))),
                                                         network_output_size=network_output_size,
                                                         batch_size=batch_size,
                                                         height=inputShape[0],
                                                         width=inputShape[1],
                                                         no_channels=no_input_channels,
                                                         use_depth=use_depth,
...
feature_volumes = leg.predict_generator(test_generator_leg, max_queue_size=10, workers=8, verbose=1)
...
model_outputs = head.predict_generator(test_generator_head, max_queue_size=10, workers=8, verbose=1)
...
diffs_orientation_yaw = np.minimum(abs(network_orientation_output - test_orientation_yaw),
                               network_output_size - abs(network_orientation_output - test_orientation_yaw))
diffs_orientation_yaw = diffs_orientation_yaw[test_overlap > 0.7]
mean_diff_yaw = np.mean(diffs_orientation_yaw)
mean_square_error_yaw = np.mean(diffs_orientation_yaw * diffs_orientation_yaw)
rms_error_yaw = np.sqrt(mean_square_error)
max_error_yaw = np.max(diffs_orientation_yaw)
...
plt.figure(2);
plt.clf();
plt.hist(diffs_orientation_yaw, bins=n_bins)
