...
class Infer:
    ...
    def __init__(self, config):
        ...
        orientation_predict_z = network_generate_method_orientation_head(inputHead_l, inputHead_r,
                                                                         'translation_output_z',
                                                                         config['model'])

        self.head = keras.Model(inputs=(inputHead_l, inputHead_r), outputs=[overlap_predict,
    ...
    def infer_one(self, filepath1, filepath2):
        ...
        pitch_out = 180 - np.argmax(model_outputs[2], axis=1)
        roll_out = 180 - np.argmax(model_outputs[3], axis=1)
        x_out = 180 - np.argmax(model_outputs[4], axis=1)
        y_out = 180 - np.argmax(model_outputs[5], axis=1)
        z_out = 180 - np.argmax(model_outputs[6], axis=1)

        return overlap_out, yaw_out, pitch_out, roll_out, x_out, y_out, z_out
    ...
    def infer_multiple(self, current_frame_id, reference_frame_id):
        ...
        if len(reference_frame_id) > 0:
            ...
            model_outputs = self.head.predict_generator(test_generator_head, max_queue_size=10, workers=8, verbose=1)
    ...
    def infer_multiple_vs_multiple(self, file_names, first_idxs, second_idxs):
        ...
        if len(second_idxs) > 0:
            ...
            test_generator_head = ImagePairOverlapSequenceFeatureVolume(pair_indizes,
                                                                        np.zeros((len(pair_indizes))),
                                                                        self.batch_size,
                                                                        np.array(self.feature_volumes))
            model_outputs = self.head.predict_generator(test_generator_head,
                                                        max_queue_size=10,
                                                        workers=8,
                                                        verbose=1)
    ...
    def create_feature_volumes(self, filenames):
        ...
        """
        generator_leg = ImagePairOverlapOrientationSequence(image_path=self.datasetpath,
                                                            imgfilenames1=filenames,
                                                            imgfilenames2=[],
                                                            dir1=[self.seq for _ in range(len(filenames))],
                                                            dir2=[],
                                                            overlap=np.zeros((len(filenames))),
                                                            orientation_yaw=np.zeros((len(filenames))),
                                                            orientation_pitch=np.zeros((len(filenames))),
                                                            orientation_roll=np.zeros((len(filenames))),
                                                            translation_x=np.zeros((len(filenames))),
                                                            translation_y=np.zeros((len(filenames))),
                                                            translation_z=np.zeros((len(filenames))),
                                                            network_output_size=self.network_output_size,
                                                            batch_size=self.batch_size,
                                                            height=self.inputShape[0],
                                                            width=self.inputShape[1],
                                                            no_channels=self.no_input_channels,
...
if __name__ == '__main__':
    configfilename = '/home/nana/Desktop/workspace/OverlapNet/config/network.yml'  # 'config/network.yml'
    ...
    if len(sys.argv) > 2:
        ...
        scan1 = '/home/nana/Desktop/workspace/OverlapNet/data/07/velodyne/000020.bin'
        scan2 = '/home/nana/Desktop/workspace/OverlapNet/data/07/velodyne/000021.bin'

        # '/home/nana/Desktop/workspace/OverlapNet/data/07/velodyne/000000.bin'

    config = yaml.load(open(configfilename))
    infer = Infer(config)

    print('Test infer one ...')
    overlap, yaw, pitch, roll, x, y, z = infer.infer_one(scan1, scan2)
    print("Overlap:  ", overlap)
    print("Orientation yaw:  ", yaw)
    print("Orientation pitch:  ", pitch)
    print("Orientation roll:  ", roll)
    ...
    overlaps, yaw, pitch, roll, x, y, z = infer.infer_multiple(1, [0])
    overlaps, yaw, pitch, roll, x, y, z = infer.infer_multiple(2, [0, 1])
    ...
    first_idxs = [0, 1, 2]
    second_idxs = [2, 1, 1]
    overlaps, yaw, pitch, roll, x, y, z = infer.infer_multiple_vs_multiple(file_names, first_idxs, second_idxs)
    print("Overlaps         :  ", overlaps)
    print("Orientation   yaw:  ", yaw)
    print("Orientation pitch:  ", pitch)
    print("Orientation  roll:  ", roll)
