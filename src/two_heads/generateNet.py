...
def generate360OutputkLegsFixed(left_input, right_input, input_shape=(50, 50, 1), config={}, smallNet=False):
    """
    Generate legs like in the DeltaLayerConv1Network.
    Here we use several Conv2D layer to resize the output of leg into 360.
...
def generateCorrelationHead(encoded_l, encoded_r, head_name, config={}):
    """
    ...
    norm_corr = Flatten(name=head_name)(norm_corr)
...
def generateSiameseNetworkTemplate(input_shape=(50, 50, 1), config={}, smallNet=False):
    ...
    (encoded_l, encoded_r) = leg_method(left_input, right_input, input_shape, config, smallNet)
    ...
    # The orientation head  ==> yaw
    head_method = getattr(sys.modules[__name__], 'generate' + config['orientation_head'])
    prediction_orientation_yaw = head_method(encoded_l, encoded_r, 'orientation_output_yaw', config)

    # this part is added by Nana
    # The orientation head ==> pitch
    head_method = getattr(sys.modules[__name__], 'generate' + config['orientation_head'])
    prediction_orientation_pitch = head_method(encoded_l, encoded_r, 'orientation_output_pitch', config)

    # The orientation head ==> row
    head_method = getattr(sys.modules[__name__], 'generate' + config['orientation_head'])
    prediction_orientation_roll = head_method(encoded_l, encoded_r, 'orientation_output_roll', config)

    # The translation head ==> x
    head_method = getattr(sys.modules[__name__], 'generate' + config['orientation_head'])
    prediction_translation_x = head_method(encoded_l, encoded_r, 'translation_output_x', config)

    # The translation head ==> y
    head_method = getattr(sys.modules[__name__], 'generate' + config['orientation_head'])
    prediction_translation_y = head_method(encoded_l, encoded_r, 'translation_output_y', config)

    # The translation head ==> z
    head_method = getattr(sys.modules[__name__], 'generate' + config['orientation_head'])
    prediction_translation_z = head_method(encoded_l, encoded_r, 'translation_output_z', config)

    # Generate a keras model out of the input and output tensors
    siamese_net = Model(inputs=[left_input, right_input],
                        outputs=[prediction_overlap,
                                 prediction_orientation_yaw,
                                 prediction_orientation_pitch,
                                 prediction_orientation_roll,
                                 prediction_translation_x,
                                 prediction_translation_y,
                                 prediction_translation_z]
                        )
    return siamese_net
...
if __name__ == "__main__":
    ...
    config['legsType'] = '360OutputkLegs'   # '360OutputkLegs_smaller'
